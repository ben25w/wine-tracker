...
function parseBulkWines() {
  ...
  document.getElementById('bulkPreview').innerHTML = bulkWines.map(wine => `
    <div class="bulk-preview-item">
      <strong>${escapeHtml(wine.name)}</strong>
      <small>${escapeHtml(wine.type)}${wine.country ? ' • ' + escapeHtml(wine.country) : ''}${wine.region ? ' • ' + escapeHtml(wine.region) : ''}</small>
    </div>
  `).join('');
